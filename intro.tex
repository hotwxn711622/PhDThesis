While safety critical software are often designed and tested with extra efforts, testing cannot
...
have functions that temporarily breaks the invariant. For example, an operation to a data structure
...
of the function correctness itself, and we think the automation engine should focus solely
on providing
...
Different abstract states are isolated by design, thus by design, primitives operating on one abstract
...
we can postpone introducing this invariants until we get to the layer where we have already
verified the full function that gracefully operates on this list preserving the invariants.
...
As a case study, we have successfully verified a practical single-core
...
multiple concurrent threads.   Recent
