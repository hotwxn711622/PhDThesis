...
effectively with our automation engine completely at logical level, and we no longer need to deal with
...
\begin{itemize}
\item We present a novel, simple, but extremely powerful automation engine
...
proving the functional correctness of each function.
To prove the effectiveness of our approach, using the new tools, we have
successfully constructed a fully verified
practical feature-rich certified OS kernels, which runs
on stock x86 hardware and doubles as a hypervisor and boots a version of Linux as guest.
...
on other concurrent programs. This allows us to reuse all of
