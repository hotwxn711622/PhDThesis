...
used to model the behaviors of hardware and the unverified bootloader.
The abstract states of the MBoot interface are shown in Fig.~\ref{fig:ref:mboot_rdata}.

On an x86-based system, the BIOS reports the physical memory information
...
Since this data is initialized by the bootloader, the
\textsf{MMInfo} in Fig.~\ref{fig:ref:mboot_rdata} also contains the default case
\textsf{MMUndef} to indicate the case when it is not yet initialized by the bootloader.
...
receives an integer $i$ as the parameter and returns appropriate field in \textsf{MM}
...
the execution returns back to the user or the guest.


\begin{figure}
	\lstinputlisting [language=Coq] {src/bootloader.v}
	\caption{The specification of \texttt{bootloader} in Coq}
...
data would only satisfy global invariants when they are properly initialized by these codes.
...
we have the specification of the bootloader, which initializes the \textsf{MM} and \textsf{MMSize}.
...
being \textsf{true}, i.e., the data are properly initialized by corresponding initialization function.
...
Since the MM table and its size comes from BIOS, the invariant is treated as the top-level
...
Figure \ref{fig:bootloader_v} presents our Coq specification for the bootloader primitive.
...
In the physical memory management module, we build a physical page allocation map to maintain the
...
this reasoning completely at the logical level, without worrying about isolation.
...
Note that as we discussed earlier, the first thing it does is to call the lower level initialization
...
Next, it sets a proper allocation state for every page based on the information available in \textsf{MM}.
...
is now completely at the logical level and there are no in-memory data involved, thus no need
...
With this information, we can utilize the automation engine to derive the main lemma shown in Figure \ref{fig:nps_loop_v}.
...
\begin{figure}
	...
	\caption{The predicate indicating whether the page is reserved or usable}
...
We could observe that the inputs for the inner loop are reasonably complicated. It is because
this inner loop is just an auxiliary loop to assist in the main goal of initializing the allocation table
...
Unlike the inner loop, the inputs for the main loop are reasonably simple.
...
In this MATInit layer interface, we also introduce the following invariant for abstract \textsf{AT}.
...
\item any other page is either reserved by BIOS or marked as usable.
\end{itemize}
\end{invariant}
...
To prove that, we need the following two lemmas.
...
Page table 0 is reserved for the kernel page table and the whole page table is set to an identity map.
...
Since we map many portions of various page tables to identity maps, we statically allocate one-page table entry
...
types and one cannot be cast to the other. Thus, in order to add the permission bits
...
This can make sure that we always pass invalid arguments, without actually hindering the performance
by doing the check at runtime.
...
the process index (specification-only) and setting the proper page table root.

The function \textsf{get\_PDE} returns the corresponding page directory entry,
...
The verification of these getters and setters are trivial and can be achieved with a single tactic call,
...
In this layer interface, we further introduce higher-level functions to allocate
...
When these validations fail, the specification gets stuck. As in the end, we prove that
no top-level system call could get stuck on any potential user input, the validations
are the expected usage of these primitives in the higher layers.
For example, the specification of \textsf{pt\_alloc\_pde}
...
called on the running thread, and the page directory we are trying to remove was properly
...
The verification of these methods is similar to the ones presented before
containing the loops and are not detailed here.

\paragraph{Layer Interface MPTKern}

Next, we introduce the two primitives \textsf{pt\_insert} and \textsf{pt\_rmv} that
...
we introduce a bitmap of size \textsf{NUM\_PROC}, to record whether each process is already spawned or
...
is formally verified for the absence of stack overflow.
...
These getter and setter functions provide arbitrary access to the TCB fields,
and thus, we cannot impose any reasonable invariant on TCBs at this layer.
...
In this layer interface, we introduce a new initialization function \textsf{thread\_init} to initialize
the TCB pool. As shown in Figure \ref{fig:tcb_c}, the thread state in every TCB is marked
dead, and the previous and next TCB indexes are set to an invalid index.
...
is represented simply as a pair of indexes in the static TCB pool representing the head and tail TCBs.
...
this intermediate abstract representation. The benefit is that this makes the verification of the TCB and queue
...
Then we verify that the layer interface PQueueInit and PAbQueue are contextually equivalent, without involving any
reasoning on the actual code, completely at the logical level.
...
In this layer, we also introduce the following set of layer invariants. Note that we do not need to include unnecessary
...
The index of any unallocated process is not in any queue, i.e., $b=-1$ in the corresponding TCB.
\end{invariant}
...
is implemented in the assembly. The implementation shown in Figure \ref{fig:thread_s}
...
Figure \ref{fig:thread_sched_v} also shows its specification.
...
At PThread layer, we have the full set of thread scheduling primitives, and we no longer expose the low
...
The layer interface PIPCIntro abstracts the IPC message into an abstract buffer and introduces the getter and setter primitives.
...
In these two layers, we introduce the Virtual Machine Extension metadata (VMX), and relevant primitives to initialize
...
in order. When we return back to the user from the system call handler, we set the error number in the register \textsf{EAX},
